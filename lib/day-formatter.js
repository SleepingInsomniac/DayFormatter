...
class DayFormatter {
  ...
  static groupConsecutiveDays(days) {
    ...
    return days.reduce((acc, day, i) => {
      ...
      let nextFromLast = DayFormatter.dayOfWeek(prevDay + 1);

      if (nextFromLast == day) { // if the previous day + 1 is equal to the current day...
        subArray.push(day); // ... add the day to the array
      } else {
        acc.push([day]); // ... create and initalize a new group
      }
